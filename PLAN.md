...
3. **Provider-dependent input products**: for the same acquisition date Planetary Computer and Earth Search can serve different processing baselines (Alibeyköy 2021-08-02: PC 03.00 original vs Earth Search 05.00 Collection-1). Baseline water differs by ~27 ha (249.60 vs 276.66 ha). Documented in README; no harmonisation of SCL/atmospheric correction across baselines is possible from the pipeline side.
4. **Scene grids must match**: `analyze` raises if the two windows differ in shape (different tile/orbit). No automatic reprojection onto a common grid yet.
